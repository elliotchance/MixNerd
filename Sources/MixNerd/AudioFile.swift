...
class AudioFile {
  ...
  func cueFileContent() -> String {
    ...
    if let tracklist = tracklist {
      content.append("REM Generated by https://github.com/elliotchance/MixNerd")
      content.append("REM Generated on \(ISO8601DateFormatter().string(from: Foundation.Date()))")

      content.append("PERFORMER \"\(tracklist.artist)\"")
      content.append("TITLE \"\(tracklist.date.description) \(tracklist.title)\"")

      // TODO: This only works for MP3 files, but right now we only support MP3 files.
      content.append("FILE \"\(audioFilePath.lastPathComponent)\" MP3")

      for (index, track) in tracklist.tracks.enumerated() {
        let trackNumber = index + 1
        content.append("  TRACK \(String(format: "%02d", trackNumber)) AUDIO")
        content.append("    PERFORMER \"\(track.artist)\"")
        content.append("    TITLE \"\(track.title)\"")
        content.append("    INDEX 01 \(track.time):00")
      }
    }
