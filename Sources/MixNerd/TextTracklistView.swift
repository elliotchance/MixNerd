...
struct TextTracklistView: View {
    ...
    VStack(spacing: 0) {
      if tracklist.shortLink != nil {
        ...
        Form {
          ...
          TextField(
            ...
            text: Binding(
              ...
              set: { newValue in
                duration = newValue
                if Time.isValidTimeString(newValue) {
                  tracklist.duration = Time(string: newValue)
                }
              }
            )
          )
          .background(Time(string: duration).at > 0 ? Color.clear : Color.red.opacity(0.3))
          .disabled(!estimateMissingTrackTimes)
        ...
        GeometryReader { geometry in
          ...
          .frame(
            width: geometry.size.width, height: geometry.size.height, alignment: .bottomTrailing
          )
  ...
  func tracklistText() -> String {
    var s = "\(tracklist.artist) - \(tracklist.title) \(tracklist.date)\n\n"

    var trackNumber = 1
    for track in tracklist.withEstimatedTrackTimes(totalTime: Time(string: duration)).tracks {
  ...
  private func copyTracklistToClipboard() {
    ...
    pasteboard.setString(tracklistText(), forType: .string)
  }
